    ...
    if(4 * nnum + 1 > p->n_nodes){
        ...
        if(in_range(loc, x, yl)){
            if(vs->count >= vs->id_array_size){
                //need to dynamic allocate space
                int32_t* temp;
                if(NULL == (temp = realloc(vs->id, vs->id_array_size * 2 * sizeof(vs->id[0])))){
                    //fail to allocate space 
                    return;
                }
                vs->id = temp;
                vs->id_array_size *= 2;
                //renew the array size
            }
            //use insertion sort to add to the id array
            int32_t i;
            for(i = vs->count ; i > 0 ; i --){
                if(p->node[nnum].id >= vs->id[i - 1]){
                    break;
                }
                vs->id[i] = vs->id[i - 1];
            }
...
* RETURN VALUE: 0 -- Failure (when the source vertexs set or destination vertex set is out of range)
*               1 -- Success
* SIDE EFFECT: none
*/
...
dijkstra (graph_t* g, heap_t* h, vertex_set_t* src, vertex_set_t* dest, path_t* path)
{
    ...
    if(src->count <= 0 ||src->count >= g->n_vertices ||
        dest->count <= 0  || dest->count >= g->n_vertices ){
    ...
    for (i = 0; i < dest->count ; i++){
        if(g->vertex[dest->id[i]].from_src < min){
            ...
            minid2 = minid1 = dest->id[i];
        }
    }
    //need to allocate space before calling
    path->tot_dist = min;
    path->n_vertices = 0;
    ...
    while(minid1 != -1){
        path->n_vertices++;
        minid1 = g->vertex[minid1].pred;
    }
    //dynamic allocate space for id array
    if(path->n_vertices == 0 || NULL == (path->id = malloc(path->n_vertices * sizeof(path->id[0])))){
        return 0;
    }
    //store the node id into the array
    for(int32_t i = path->n_vertices; i > 0 && minid2 != -1 ; i--){
        path->id[i - 1] = minid2;
        minid2 = g->vertex[minid2].pred;
    }
